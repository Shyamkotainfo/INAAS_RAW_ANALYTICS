...
import { ChatResponse, ChartData } from '../config/api';
import ChartRenderer from './ChartRenderer';
import DocumentIcon from "../asserts/Download-icon.svg";
import logo from "../asserts/info-sevices-logo.png";
import { Mic, Send } from "lucide-react";
import { Dialog, DialogTitle, DialogContent, DialogActions, Button, Box, Typography, FormControl, RadioGroup, FormControlLabel, Radio, Divider } from '@mui/material';
...
const ChatInterface: React.FC = () => {
  ...
  const [openDownloadResultId, setOpenDownloadResultId] = useState<string | null>(null);
  ...
  const [isGlobalDownloadOpen, setIsGlobalDownloadOpen] = useState<boolean>(false);
  ...
  const [axisSelections, setAxisSelections] = useState<Record<string, { x?: string; y?: string }>>({});

  const [isListening, setIsListening] = useState(false);
  const [recognition, setRecognition] = useState<MinimalSpeechRecognition | null>(null);
  ...
  const [dialogApiData, setDialogApiData] = useState<{ user_input: string; chat_response: string }>({ user_input: '', chat_response: '' });
  ...
  const latestApiData = useMemo(() => {
    ...
    const lastResponse = responses[responses.length - 1]?.response as unknown as { data?: Record<string, unknown> };
  ...
  const chartTypeOptions: Array<{ value: ChartData['type']; label: string; description: string; icon: string }> = [
    { value: 'bar', label: 'Bar Chart', description: 'For categorical comparisons', icon: '📊' },
    { value: 'line', label: 'Line Chart', description: 'For time series and trends', icon: '📈' },
    { value: 'pie', label: 'Pie Chart', description: 'For proportional data', icon: '🥧' },
    { value: 'doughnut', label: 'Doughnut Chart', description: 'For proportional data with center space', icon: '🍩' },
    { value: 'radar', label: 'Radar Chart', description: 'For multi-dimensional data comparison', icon: '🕸️' },
    { value: 'polarArea', label: 'Polar Area Chart', description: 'For circular proportional data', icon: '⚪' },
    { value: 'bubble', label: 'Bubble Chart', description: 'For 3D data visualization', icon: '🫧' },
    { value: 'scatter', label: 'Scatter Chart', description: 'For correlation analysis', icon: '🔍' }
  ];
  ...
  const quickQueryColors = ['#2563eb', '#16a34a', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4'];

  const quickPageSize = 5;
  const quickTotalPages = Math.max(1, Math.ceil(quickQueries.length / quickPageSize));
  ...
  useEffect(() => {
    const initSpeechRecognition = () => {
      try {
        ...
        recognitionInstance.onresult = (event: MinimalSpeechRecognitionEvent) => {
          let finalTranscript = '';
  ...
  const handleSubmit = async (e: React.FormEvent, preferredCatalog: string = '', overridePrompt?: string) => {
    e.preventDefault();
    ...
    try {
      // Use real API service with preferred_catalog
      const chatResponse = await apiService.sendChatPrompt(currentPrompt, preferredCatalog);
      ...
      const rawDataArray = Array.isArray(responseData.rawData) ? responseData.rawData : undefined;
      const resultsCandidate = (responseData as { results?: unknown[] }).results;
      const resultsArray = Array.isArray(resultsCandidate) ? resultsCandidate : undefined;
      ...
      const shouldCreateResultCard = chatResponse.success && (hasRawData || hasResultsData || hasInsights);
      ...
      if (shouldCreateResultCard) {
        ...
        setResponses(prev => [...prev, newResult]);
  ...
  const handleCatalogSelection = async () => {
    ...
    // Submit with the selected catalog using pendingPrompt directly
    // const fakeEvent = { preventDefault: () => { } } as React.FormEvent;
    setIsLoading(true);

    try {
      const chatResponse = await apiService.sendChatPrompt(pendingPrompt, selectedCatalog);
  ...
  const handleChartTypeChangeAll = (
    ...
    setResponses(prev => prev.map(result => {
      ...
      const updatedTypes: { [key: number]: ChartData['type'] } = { ...result.chartTypes };
  ...
  const generateChartsFromData = (
    ...
    const numericKeys = keys.filter(key => typeof sampleItem[key] === 'number');
    const stringKeys = keys.filter(key => typeof sampleItem[key] === 'string');
    ...
    const isTimeLabelKey = (key: string): boolean => /^(period|date|day|week|month|year|time)$/i.test(key) || /date|day|period|week|month|year|time/i.test(key);
    ...
    if (numericKeys.length > 0 && stringKeys.length > 0) {
      ...
      if (axisSelections?.y === 'reconnecting_users' && axisSelections?.x) {
        ...
        labelKey = axisSelections.x;
        const preferredNumericOrder = ['reconnecting_users', 'churned_users', 'count', 'value'];
        valueKey = preferredNumericOrder.find(k => numericKeys.includes(k)) || numericKeys[0];
      ...
      const looksTimeSeries = isTimeLabelKey(labelKey) || isDateLikeValue(firstLabelValue);
      ...
      if (axisSelections?.x && !axisSelections?.y) {
        // Only X selected - use actual X values (e.g., period like "2025-04") as labels
        chartLabels = rawData.map(item => String((item as Record<string, unknown>)[labelKey]));
        ...
        chartLabels = rawData.map(item => String((item as Record<string, unknown>)[labelKey]));
        chartData = rawData.map(item => Number((item as Record<string, unknown>)[valueKey]));
      } else if (axisSelections?.x && axisSelections?.y) {
        ...
        if (xIsNumeric && yIsString) {
          ...
          if (mainChartType === 'line') {
            ...
            charts.push({
              ...
              data: {
                ...
                datasets: [{
                  label: swappedValueKey.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
            ...
            chartLabels = rawData.map(item => String((item as Record<string, unknown>)[swappedLabelKey]));
            chartData = rawData.map(item => Number((item as Record<string, unknown>)[swappedValueKey]));
            charts.push({
              type: 'bar',
              title: `Chat ${baseIndex + 1} - Data Analysis`,
              data: {
                ...
                datasets: [{
                  label: swappedValueKey.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
                  data: chartData,
                  backgroundColor: chartData.map((_, index) => `hsl(${(index * 137.5) % 360}, 70%, 60%)`),
                  borderColor: chartData.map((_, index) => `hsl(${(index * 137.5) % 360}, 70%, 50%)`),
                  borderWidth: 2,
                }]
              },
              options: { indexAxis: 'y' }
        ...
        } else if (stringKeys.includes(axisSelections.x) && stringKeys.includes(axisSelections.y) && axisSelections.x === axisSelections.y && isTimeLabelKey(axisSelections.x)) {
          // Both X and Y are the same time-like field (e.g., period vs period)
          const key = axisSelections.x;
          const labels = rawData.map(item => String((item as Record<string, unknown>)[key]));
          ...
          return charts;
        } else if (!yIsNumeric && stringKeys.includes(axisSelections.x)) {
          // X categorical, Y categorical -> count occurrences per X label
          const xKey = axisSelections.x;
          const counts = new Map<string, number>();
          for (const item of rawData as Array<Record<string, unknown>>) {
            const x = String(item[xKey] ?? '');
            counts.set(x, (counts.get(x) ?? 0) + 1);
          }
          chartLabels = Array.from(counts.keys());
          chartData = chartLabels.map(lbl => counts.get(lbl) ?? 0);
        } else {
      ...
      const primaryChart: ChartData = {
        ...
        data: {
          labels: chartLabels,
          datasets: [{
            label: axisSelections?.x && !axisSelections?.y
              ? labelKey.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
              : valueKey.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
            data: chartData,
            backgroundColor: chartData.map((_, index) => `hsl(${(index * 137.5) % 360}, 70%, 60%)`),
            borderColor: chartData.map((_, index) => `hsl(${(index * 137.5) % 360}, 70%, 50%)`),
            borderWidth: 2,
      ...
      const shouldShowComparisonChart = numericKeys.length > 1 && !axisSelections?.y;

      if (shouldShowComparisonChart) {
        const comparisonChartType: ChartData['type'] = resultChartTypes[0] || (looksTimeSeries ? 'line' : 'bar');
        const comparisonChart: ChartData = {
          type: comparisonChartType,
          title: `Chat ${baseIndex + 1} - Multiple Metrics`,
          data: {
            labels: rawData.map(item => (item as Record<string, unknown>)[labelKey]),
            datasets: numericKeys.map((key, index) => ({
              label: key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
              data: rawData.map(item => Number((item as Record<string, unknown>)[key])),
      ...
      const distributionChartType: ChartData['type'] = resultChartTypes[0] || 'pie';
      charts.push({
        ...
        data: {
          ...
          datasets: [{
            data: rawData.map(item => numericKeys[0] ? Number((item as Record<string, unknown>)[numericKeys[0]]) : 0),
            backgroundColor: rawData.map((_, index) => `hsl(${(index * 137.5) % 360}, 70%, 60%)`),
            borderColor: rawData.map((_, index) => `hsl(${(index * 137.5) % 360}, 70%, 50%)`),
            borderWidth: 2,
          }]
        }
      });
  ...
  const handleDownloadSummary = (
    result: { response?: { data?: Record<string, unknown> }; resultId?: string }
  ) => {
  ...
  const handleDownloadData = (result: { rawData?: unknown[]; resultId?: string }) => {
  ...
  const handleDownloadAll = async (
    result: { resultId?: string; response?: { data?: Record<string, unknown> }; rawData?: unknown[] }
  ) => {
    try {
      ...
      type JsPDFConstructor = new (options?: { unit?: string; format?: string | [number, number] }) => JsPDFInstance;
      ...
      const ensureAutoTable = async () => {
        const api = (JsPDFCtor as unknown as { API?: { autoTable?: unknown } } | undefined)?.API;
        const hasPlugin = typeof api?.autoTable === 'function';
        if (!hasPlugin) {
          await new Promise<void>((resolve, reject) => {
            const script = document.createElement('script');
            script.src = 'https://cdn.jsdelivr.net/npm/jspdf-autotable@3.8.1/dist/jspdf.plugin.autotable.min.js';
            script.async = true;
            script.onload = () => resolve();
      ...
      try {
        const rawUserInput = (result?.response?.data?.['user_input'] as unknown) ?? 'No Query Information Available';
        ...
        doc.text(formattedUserInput, pageWidth / 2, cursorY, { align: 'center' });
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(0);
        // Slightly smaller gap before charts
        const selChartPadTop = 12;
        cursorY += selChartPadTop;
      } catch { /* ignore formatting errors and continue */ }
      ...
      // Charts section FIRST
      {
        ...
        const container = document.querySelector(`[data-result-id="${result?.resultId}"]`);
        const canvases = container ? Array.from(container.querySelectorAll('canvas')) : [];
        const targetCanvas = canvases[2] || canvases[0];
        if (targetCanvas) {
          const dataUrl = (targetCanvas as HTMLCanvasElement).toDataURL('image/png', 1.0);
          const chartScale = 0.7; // reduce visual size
          const maxImgWidth = (pageWidth - marginX * 2) * chartScale;
          const aspect = (targetCanvas as HTMLCanvasElement).height / (targetCanvas as HTMLCanvasElement).width;
          ...
          doc.addImage(dataUrl, 'PNG', marginX, cursorY, drawWidth, drawHeight, undefined, 'FAST');
      ...
      const summaryText = formatInsightsToText(summaryRaw) || 'No insights provided.';
      ...
      for (const rawLine of summaryLinesArray) {
        ...
        if (match) {
          ...
          doc.setFont('helvetica', 'normal');
          const wrappedContent = doc.splitTextToSize(content, Math.max(10, maxWidth - labelWidth));
      ...
      cursorY += 24;
      const dataArr: Array<Record<string, unknown>> = Array.isArray(result?.rawData)
        ? (result.rawData as Array<Record<string, unknown>>)
        : [];
      doc.setFontSize(12);
      doc.setFont('helvetica', 'bold');
      ...
      if (dataArr.length > 0 && typeof dataArr[0] === 'object') {
        ...
        const body = dataArr.map((row: Record<string, unknown>) => headers.map(h => {
    ...
    try {
      ...
      responses.forEach((result, index) => {
        ...
        try {
          ...
          doc.text(formattedUserInput, pageWidthAll / 2, cursorY, { align: 'center' });
          doc.setFont('helvetica', 'normal');
          doc.setTextColor(0);
          cursorY += 20;
        } catch { /* ignore */ }
        ...
        if (canvases.length > 0) {
          canvases.forEach((canvas) => {
            ...
            const dataUrl = (canvas as HTMLCanvasElement).toDataURL('image/png', 1.0);
            const chartScale = 0.68; // smaller in all-responses
            const maxImgWidth = (pageWidth - marginX * 2) * chartScale;
            const aspect = (canvas as HTMLCanvasElement).height / (canvas as HTMLCanvasElement).width;
            const availableHeight = Math.max(60, pageHeight - 60 - cursorY);
      ...
      const filename = `complete-report-${new Date().toISOString().split('T')[0]}.pdf`;
      doc.save(filename);
    } catch (err) {
  ...
  useEffect(() => {
    (window as unknown as { __downloadAllResponses?: unknown }).__downloadAllResponses = handleDownloadAllResponses;
    return () => {
      delete (window as unknown as { __downloadAllResponses?: unknown }).__downloadAllResponses;
    };
  ...
  useEffect(() => {
    ...
    for (const res of responses) {
      ...
      if (Array.isArray(rows) && rows.length > 0 && typeof rows[0] === 'object') {
        ...
        if (headers.length > 2 && !autoTableShownRef.current.has(res.resultId)) {
  ...
  const toggleSingleBulk = (resultId: string, checked: boolean) => {
    setBulkSelectedIds(prev => {
      ...
      const allSelected = responses.length > 0 && responses.every(r => updated[r.resultId]);
  ...
  const handleDownloadSelectedResponses = async () => {
    ...
    try {
      ...
      selected.forEach((result, index) => {
        ...
        cursorY += 6;
        try {
          const formattedUserInput = formatUserInputTitle(result?.response?.data?.user_input);
          doc.setFontSize(14);
          doc.setFont('helvetica', 'bold');
          doc.setTextColor(44, 90, 160);
          const pageWidthAll: number = doc.internal.pageSize.getWidth();
      ...
      const filename = `selected-report-${new Date().toISOString().split('T')[0]}.pdf`;
      doc.save(filename);
      setIsGlobalDownloadOpen(false);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col overflow-x-hidden" style={{ backgroundColor: '#f5f5f5' }}>
      {/* Header */}
      <header className="bg-white shadow-sm sticky top-0 z-30" style={{ borderBottom: '2px solid #e0e0e0' }}>
        <div className="" style={{ padding: '10px' }}>
          <div className="flex items-center space-x-0  ">
            <Image
              ...
              height={35}
            />
            {/* <p className="text-xll font-semibold uppercase" style={{ color: '#2c5aa0' }}>
            ...
            {/* <div className="flex flex-col items-center">
              ...
              <p className="text-sm font-semibold text-gray-700">Insight as a Service</p>
            </div> */}
      ...
      <main className="flex-1 flex flex-col">
        {/* Input Section */}
        <div
          className={`transition-all duration-500 ease-in-out ${isLanding ? "flex-1 flex items-center justify-center px-6 min-h-0" : "px-4 py-2"}`}
          style={{
        ...
        >
          <div className={`w-full ${isLanding ? "max-w-5xl mx-auto" : "max-w-7xl mx-auto"}`}>
            {responses.length === 0 && !isLoading ? (
              <div className="rounded-2xl p-6" style={{ background: 'linear-gradient(135deg, #fff7f7 0%, #fffaf0 25%, #f5fff7 50%, #f0f9ff 75%, #f8f5ff 100%)', boxShadow: '0 6px 16px rgba(59, 130, 246, 0.18), 0 12px 28px rgba(236, 72, 153, 0.16), 0 18px 44px rgba(16, 185, 129, 0.14), 0 24px 64px rgba(99, 102, 241, 0.12)', minHeight: '380px', marginBottom: '30px' }}>
                <div className="flex justify-center">
                  <p className="text-4xl font-semibold uppercase" style={{ color: '#2c5aa0', marginTop: '15px' }}>
                    Insight as a Service
                  </p>
                </div>
                <div className="mb-6 text-center mt-2">

                  <h1 className="text-2xl md:text-3xl font-bold text-gray-800">Data to Decisions</h1>

                  <p className="text-xl text-gray-600 max-w-2xl mx-auto">
                    Discover patterns, track trends, and insights effortlessly.
                  </p>
                </div>
                      ...
                      <div className="relative">
                        <textarea
                          ...
                          style={{
                            ...
                            borderColor: hasInterimResults ? "#ff9800" : "#e0e0e0",
                            backgroundColor: hasInterimResults ? "#fff8f0" : "#ffffff",
                            color: "#333",
                            fontSize: "14px",
                          }}
                        ...
                        />
                        <div className={`absolute right-4 bottom-4 flex items-center gap-3 ${isInputUsed ? "" : ""}`}>
                          <button
                            type="button"
                            onClick={handleSpeechRecognition}
                            title={isListening ? "Stop listening" : "Start speech recognition"}
                            className={`p-3 rounded-full transition-colors ${isListening ? "bg-red-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                              }`}
                          >
    ...
    `}
                        ...
                        <div className="marquee-container overflow-hidden" style={{ width: "970px" }}>
                          <div
                            ...
                            {quickQueries.map((q, idx) => (
                              ...
                              >
                                <span
                                  ...
                                  style={{ backgroundColor: quickQueryColors[idx % quickQueryColors.length] }}
                                />
                                {q}
                              </button>
                            ))}
                      ...
                      {isListening && (
                        <div
                          ...
                          style={{ backgroundColor: "rgba(255, 255, 255, 0.15)", borderLeft: "4px solid #e74c3c" }}
                        >
                          <span className="text-red-500 animate-pulse text-sm">🔴</span>
                          <div className="flex-1">
                            <span className="text-white text-sm font-medium" style={{ textShadow: "0 1px 2px rgba(0,0,0,0.3)" }}>
                              {hasInterimResults ? "Live transcribing..." : "Listening..."}
                            </span>
                            {hasInterimResults && (
                              <div className="text-white text-xs opacity-80 mt-1" style={{ textShadow: "0 1px 2px rgba(0,0,0,0.3)" }}>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-1">
                        <div className="flex items-center gap-2 w-full">
                          <button
                            type="button"
                            className={`w-7 h-7 flex items-center justify-center text-xs rounded border ${quickPage === 0 ? 'opacity-40 cursor-not-allowed' : 'hover:bg-gray-50'}`}
                            onClick={(e) => { e.preventDefault(); setQuickPage((p) => Math.max(0, p - 1)); }}
                          ...
                          <div className="flex gap-2 flex-1 justify-center">
                            {visibleQuickQueries.map((q, localIdx) => {
                              const idx = quickPage * quickPageSize + localIdx;
                              return (
                                <button
                                  key={`${q}-${idx}`}
                                  type="button"
                          ...
                          <button
                            type="button"
                            className={`w-7 h-7 flex items-center justify-center text-xs rounded border ${quickPage >= quickTotalPages - 1 ? 'opacity-40 cursor-not-allowed' : 'hover:bg-gray-50'}`}
                            onClick={(e) => { e.preventDefault(); setQuickPage((p) => Math.min(quickTotalPages - 1, p + 1)); }}
            ...
            >
              <div className="px-6 border-b" style={{ borderColor: "#e0e0e0", marginTop: "-2px" }}>
                <div className="flex items-center justify-between" style={{ marginTop: "-8px" }}>
                  {/* Left side - Title */}
                  <h2 className="text-lg font-semibold" style={{ color: "#2c5aa0" }}>
                    Insights
                  </h2>
                  ...
                  <div className="relative" data-global-download-container="true">
                    ...
                    {isGlobalDownloadOpen && (
                      <div className="absolute right-0 mt-2 bg-white border rounded-lg shadow-lg w-80 text-sm z-20" style={{ borderColor: "#e0e0e0" }}>
                        <div className="px-3 py-2 border-b" style={{ borderColor: "#e0e0e0" }}>
                        ...
                        <div className="max-h-36 overflow-auto">
                          {responses.length === 0 ? (
                            ...
                            responses.slice().reverse().map((result) => {
                              const title = formatUserInputTitle(result.response?.data?.user_input);
                              return (
                                <label key={`bulk-${result.resultId}`} className="flex items-start gap-2 px-3 py-2 hover:bg-gray-50 cursor-pointer">
                                  <input
                                    type="checkbox"
                                    checked={!!bulkSelectedIds[result.resultId]}
                                    onChange={(e) => toggleSingleBulk(result.resultId, e.target.checked)}
                                    style={{ accentColor: '#2c5aa0' }}
                                  />
                                  <div>
                                    <p className="text-sm font-medium" style={{ color: '#2c5aa0' }}>{title}</p>
                                  </div>
                                </label>
                              );
                            })
                        ...
                        <div className="flex gap-2 p-3 border-t" style={{ borderColor: "#e0e0e0" }}>
                          <button
                            ...
                            onClick={(e) => { e.preventDefault(); setIsGlobalDownloadOpen(false); }}
                          >
                            Cancel
                          </button>
                          <button
                            ...
                            style={{ backgroundColor: '#2c5aa0' }}
                            onClick={(e) => { e.preventDefault(); handleDownloadSelectedResponses(); }}
                          >
                            Download
                          </button>
                        </div>
              ...
              <div className="p-2 space-y-8 max-h-[60vh] overflow-y-auto" ref={scrollContainerRef}>
                    ...
                    return (
                      <div key={result.resultId} className="fade-in-up">
                        ...
                        <div className="text-center mb-1 -mt-3">
                          <p className="text-lg font-semibold" style={{ color: '#2c5aa0' }}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto] gap-2 items-stretch">
                          ...
                          <div className="relative h-full" data-result-id={result.resultId}>
                            {/* kebab menu removed */}
                            <div className="bg-white rounded-2xl shadow-sm p-6 h-full" style={{ border: "2px solid #e0e0e0" }}>
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                  <div
                                    ...
                                    dangerouslySetInnerHTML={{
                                      __html: (result.response?.data?.insights || 'No insights provided.')
                                ...
                                <div className="flex flex-col gap-4" style={{ overflow: 'visible' }}>
                                  {showTableInChat === result.resultId ? (
                                    /* ✅ Normal Table Display (same as original look) */
                                    <div className="h-full ml-4">
                                      <div className="mb-4">
                                        <h3 className="text-lg font-semibold" style={{ color: '#2c5aa0' }}>
                                          Data Table
                                        </h3>
                                      </div>

                                      <div className="overflow-auto max-h-80">
                                        {Array.isArray(result.rawData) && result.rawData.length > 0 && typeof result.rawData[0] === 'object' ? (
                                          (() => {
                                            const typedRows = result.rawData as Array<Record<string, unknown>>;
                                            const firstRow = typedRows[0] as Record<string, unknown>;
                                    ...
                                    charts.map((chartData: ChartData, chartIndex: number) => (
                                      <div key={chartIndex} className="pr-6" style={{ overflow: 'visible' }}>
                                        <ChartRenderer
                                          chartData={{
                                            ...chartData,
                            ...
                            >
                              <div className="flex flex-col items-stretch justify-start text-base p-3 gap-3 w-48">
                                ...
                                <div className="border border-blue-400 rounded-lg p-3 bg-white shadow-sm space-y-3 hover:bg-blue-50 transition-colors">
                                  <div >
                                    <div className="text-sm font-bold mb-2">Visualization</div>
                                    {(() => {
                                      const chartsForCount = generateChartsFromData(
                                        result.rawData,
                                        result.chartTypes as { [key: number]: ChartData['type'] },
                                        result.response,
                                        resultIndex,
                                      ...
                                      const chartsCount = chartsForCount.length;
                                      const inferredType = chartsForCount[0]?.type;
                                      const currentType = (result.chartTypes && result.chartTypes[0]) || inferredType || 'bar';
                                      return (
                                        <select
                                          ...
                                          value={showTableInChat === result.resultId ? 'table' : currentType}
                                          onChange={(e) => {
                                            const sel = e.target.value;
                                            if (sel === 'table') {
                                              const willShow = showTableInChat !== result.resultId;
                                              setShowTableInChat(willShow ? result.resultId : null);
                                              return;
                                            }
                                            // switching chart type hides table if open
                                            if (showTableInChat === result.resultId) {
                                              setShowTableInChat(null);
                                            }
                                  ...
                                  {Array.isArray(result.rawData) && result.rawData.length > 0 && typeof result.rawData[0] === 'object' && (
                                    (() => {
                                      const typedRows = result.rawData as Array<Record<string, unknown>>;
                                      const sampleRow = typedRows[0] as Record<string, unknown>;
                                      const allKeys = Object.keys(sampleRow);
                                      const stringKeys = allKeys.filter((k) => typeof sampleRow[k] === 'string');
                                      const numericKeys = allKeys.filter((k) => typeof sampleRow[k] === 'number');
                                      const sel = axisSelections[result.resultId] || {};
                                      ...
                                      const timeLike = allKeys.filter(k => /^(period|date|day|week|month|year|time)$/i.test(k));
                                      const metricLike = allKeys.filter(k => /^(total_?unique_?viewers|value|count|total|amount|sum|metric|quantity|score)$/i.test(k));
                                      const defaultX =
                                        (timeLike[0]) ||
                                        (stringKeys[0]) ||
                                        (allKeys[0] || '');
                                      const defaultYCandidateOrder = [
                                        ...metricLike,
                                        ...numericKeys.filter(k => k !== defaultX),
                                        ...allKeys.filter(k => k !== defaultX)
                                      ];
                                      const defaultY = defaultYCandidateOrder[0] || '';

                                      return (
                                        <>
                                          {/* X Axis Dropdown */}
                                          <div className="w-full">
                                            <label className="block text-gray-700 text-sm font-bold mb-1">X axis</label>
                                            <select
                                              className="w-full border border-gray-300 rounded-lg px-2 py-1 text-xs text-gray-700 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-200"
                                              value={sel.x ?? defaultX}
                                              onChange={(e) =>
                                                setAxisSelections((prev: Record<string, { x?: string; y?: string }>) => ({
                                                  ...prev,
                                                  [result.resultId]: { ...(prev[result.resultId] || {}), x: e.target.value || undefined },
                                                }))
                                              }
                                            >
                                              {allKeys.map((k) => (
                                          ...
                                          {/* Y Axis Dropdown */}
                                          <div className="w-full">
                                            <label className="block text-gray-700 text-sm font-bold mb-1">Y axis</label>
                                            <select
                                              className="w-full border border-gray-300 rounded-lg px-2 py-1 text-xs text-gray-700 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-200"
                                              ...
                                              onChange={(e) =>
                                                setAxisSelections((prev: Record<string, { x?: string; y?: string }>) => ({
                                                  ...prev,
                                                  [result.resultId]: { ...(prev[result.resultId] || {}), y: e.target.value || undefined },
                                                }))
                                              }
                                            >
                                              {allKeys.map((k) => (
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    const willOpen = openSqlResultId !== result.resultId;
                                    setOpenSqlResultId(willOpen ? result.resultId : null);
                                    setOpenMenuResultId(null);
                                    setOpenDownloadResultId(null);
                                  }}
                                >
                                ...
                                <button
                                  ...
                                  onClick={() => {
                                    const willOpen = openDownloadResultId !== result.resultId;
                                    setOpenDownloadResultId(willOpen ? result.resultId : null);
                                    setOpenMenuResultId(null);
                                  }}
                                >
                                  Download
                              ...
                              {openSqlResultId === result.resultId && (
                                ...
                                >
                                  <div
                                    className="flex items-center justify-between px-3 py-2 border-b text-gray-700 border border-yellow-400 bg-yellow-50 rounded-t-lg shadow-sm"
                                  >
                                    <span className="font-medium text-yellow-700">SQL</span>
                                    <div className="flex items-center gap-2 relative">
                                      <button
                                        ...
                                        onClick={() => {
                                          const sql =
                                            (result.response?.data as { sql?: string } | undefined)?.sql?.replace(/^sql\s*\n?/, "") ||
                                            "";

                                          if (sql) {
                                            // Ensure copied SQL starts with "-- sql"
                                            ...
                                            navigator.clipboard
                                              ...
                                              .then(() => {
                                                ...
                                                setTimeout(() => setCopiedId(null), 1500); // hide tooltip after 1.5s
                                              })
                                              .catch(() => { });
                                          }
                                        }}
                                      ...
                                      <button
                                        ...
                                        onClick={() => setOpenSqlResultId(null)}
                                  ...
                                  <div className="max-h-80 overflow-auto p-3 bg-gray-50">
                                    <pre className="language-sql text-xs rounded-md p-2">
                                      <code className="language-sql">
                                        {((result.response?.data as { sql?: string })?.sql?.replace(/^sql\s*\n?/, "")
                                          || "No SQL available.")}
                                      </code>
                                    </pre>
                                  </div>
                                ...
                                >
                                  <div className="max-h-60 overflow-auto py-2">
                                    <button
                                      ...
                                      onClick={() => { handleDownloadSummary(result); setOpenDownloadResultId(null); }}
                                    >
                                      <span className="mr-2">📄</span> Download summary
                                    </button>

                                    <button
                                      type="button"
                                      className="w-full flex items-center text-left px-3 py-2 hover:bg-gray-50"
                                      onClick={() => { handleDownloadData(result); setOpenDownloadResultId(null); }}
                                    >
                                    ...
                                    <button
                                      type="button"
                                      className="w-full flex items-center text-left px-3 py-2 hover:bg-gray-50"
                                      onClick={() => { handleDownloadAll(result); setOpenDownloadResultId(null); }}
                                    >
                                      <span className="mr-2">📦</span> Download all
                                    </button>
                                  </div>
                                </div>
                              )}
        ...
        <Dialog
          open={isErrorOpen}
          onClose={() => { setIsErrorOpen(false); setErrorMessage(''); setDialogApiData({ user_input: '', chat_response: '' }); }}
          maxWidth="xs"
          fullWidth
          PaperProps={{
            sx: {
        ...
        >
          <DialogContent sx={{ textAlign: "center", p: 2.5 }}>
            <Box display="flex" gap={1} mb={1}>
              <Typography
                sx={{ fontSize: "15px", color: "#2D3748" }}
              >
                <strong>User Input:</strong> {dialogApiData.user_input || latestApiData.user_input || 'Device error'}
              </Typography>
            </Box>
          ...
          <DialogActions sx={{ justifyContent: "center", pb: 2 }}>
            <Button
              onClick={() => { setIsErrorOpen(false); setErrorMessage(''); }}
              variant="contained"
              sx={{
      ...
      >
        <DialogTitle sx={{ textAlign: 'center', pb: 1.5 }}>
          <Box display="flex" alignItems="center" justifyContent="center" gap={1}>
            ...
            <Typography sx={{ fontWeight: 600, color: '#2c5aa0' }}>Select Catalog</Typography>
          </Box>
          <Typography variant="body2" sx={{ color: 'text.secondary', mt: 1 }}>Multiple catalogs found. Choose one:</Typography>
        </DialogTitle>
        <Divider />
            ...
            >
              {catalogOptions.map((catalog) => (
                <FormControlLabel
                  key={catalog}
                  value={catalog}
                  control={<Radio sx={{ color: '#2c5aa0', '&.Mui-checked': { color: '#2c5aa0' } }} />}
                  label={
                    <Typography sx={{ color: '#2c5aa0', fontWeight: 500, fontSize: 14 }}>
                      {catalog.replace(/_/g, ' ').replace(/\b\w/g, (l: string) => l.toUpperCase())}
                    </Typography>
                  }
                  sx={{
                    m: 0,
                    p: 1,
                    border: '1px solid',
                    borderColor: selectedCatalog === catalog ? '#2c5aa0' : '#e5e7eb',
                    borderRadius: 1.5,
                    mb: 1,
        ...
        <DialogActions sx={{ px: 3, pb: 2, pt: 0, gap: 1 }}>
          ...
          <Button
            ...
            sx={{ backgroundColor: '#2c5aa0', '&:hover': { backgroundColor: '#244a86' } }}
          >
            Continue
          </Button>
        </DialogActions>
